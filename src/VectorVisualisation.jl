...
import Makie
MakiePoint = Makie.AbstractPlotting.Point
import Makie: arrows, arrows!, linesegments, linesegments!, LineSegments, Rect3D, Scene, convert_arguments, AbstractPlot, Arrows, cameracontrols, PointBased
...
function convert_arguments(::Type{<: LineSegments}, positions::AbstractVector{E}) where E <: Union{Pair{A, A}, Tuple{A, A}} where A <: Point{T, N} where {T, N}
    (Base.convert(AbstractArray{MakiePoint{N,Float32},1}, reinterpret(MakiePoint{N,T}, positions)),)
end
